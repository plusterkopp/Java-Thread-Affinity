...
import net.openhft.affinity.IGroupAffinity;
...
public enum WindowsJNAAffinity implements IAffinity, IGroupAffinity, IDefaultLayoutAffinity {
	INSTANCE {
		...
		public void setAffinity(int cpuid) {
			WindowsCpuLayout layout = (WindowsCpuLayout) getDefaultLayout();
			WindowsCpuInfo cpuInfo = (WindowsCpuInfo) layout.lCpu(cpuid);
			setGroupAffinity(cpuInfo.getGroupId(), cpuInfo.getMask());
		}
...
//            sb.append( " at: " + this.getPointer());
			switch (relationShip) {
				case WinNT.LOGICAL_PROCESSOR_RELATIONSHIP.RelationProcessorCore:
					sb.append(" CoreInfo ").append(_u.processor);
					break;
				case WinNT.LOGICAL_PROCESSOR_RELATIONSHIP.RelationProcessorPackage:
					sb.append(" PackageInfo ").append(_u.processor);
					break;
				case WinNT.LOGICAL_PROCESSOR_RELATIONSHIP.RelationNumaNode:
					sb.append(" NodeInfo ").append(_u.numaNode);
					break;
				case WinNT.LOGICAL_PROCESSOR_RELATIONSHIP.RelationCache:
					sb.append(" CacheInfo ").append(_u.cache);
					break;
				case WinNT.LOGICAL_PROCESSOR_RELATIONSHIP.RelationGroup:
					sb.append(" GroupInfo ").append(_u.group);
					break;
			}
			return sb.toString();
		}
