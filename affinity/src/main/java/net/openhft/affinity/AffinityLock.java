...
public class AffinityLock {
	...
	static {
		...
		if ( layout != null) {
			...
			for ( int i = 0; i < PROCESSORS; i++) {
				AffinityLock al = LOCKS[ i] = new AffinityLock( i,
						( ( BASE_AFFINITY >> i) & 1) != 0,
						( ( RESERVED_AFFINITY >> i) & 1) != 0);
	 ...
	 * <p/>
	 * Changing the layout will have no impact on thread which have already been assigned. It only affects subsequent
	 * assignments.
	 *
	 * @param cpuLayout
	...
	private static long getReservedAffinity0() {
		...
		if ( reservedAffinity == null || reservedAffinity.trim().isEmpty()) {
			...
			if ( reserverable == 0 && PROCESSORS > 1) {
				LOGGER.log( Level.INFO, "No isolated CPUs found, so assuming CPUs 1 to " + ( PROCESSORS - 1)
						+ " available.");
	 ...
	 *
	 * @param bind
	 *            if true, bind the current thread, if false, reserve a cpu which can be bound later.
			...
			for ( AffinityStrategy strategy : strategies) {
				...
				for ( int i = LOCKS.length - 1; i > 0; i--) {
					...
					if ( al.canReserve() && ( cpuId < 0 || strategy.matches( cpuId, al.cpuId))) {
						...
						if ( ! al.canReserve()) {
							LOGGER.info( "Lock " + al.dumpLock() + " not reservable");
						} else if ( ! strategy.matches( cpuId, al.cpuId)) {
							LOGGER.info( "No Match " + strategy + cpuId + " to " + al.dumpLock());
						}
					}
				}
			}
	 ...
	 */
	public static AffinityLock acquireLock( boolean bind, @NotNull AffinityStrategy strategy, int... cpuIds) {
		synchronized ( AffinityLock.class) {
			NextLock: for ( int i = LOCKS.length - 1; i > 0; i--) {
				AffinityLock al = LOCKS[ i];
				if ( al.canReserve()) {
	...
	private static AffinityLock acquireCore( boolean bind, int cpuId, @NotNull AffinityStrategy... strategies) {
		synchronized ( AffinityLock.class) {
			for ( AffinityStrategy strategy : strategies) {
				LOOP: for ( AffinityLock[] als : CORES.descendingMap().values()) {
	...
	public String dumpLock() {
		return dumpLock( new StringBuilder());
	}

	private String dumpLock( StringBuilder sb) {
		final int cpuID = cpuId();
		final CpuLayout cpuLayout = AffinityLock.cpuLayout();
		if ( cpuID == NONE.cpuId()) {
			sb.append( "could not bind ");
		} else {
			sb.append( cpuID).append( " ").append( cpuLayout.socketId( cpuID)).append( "/").append( cpuLayout.coreId( cpuID)).append( "/").append( cpuLayout.threadId( cpuID)).append( ": ");
		}
	...
	public static AffinityLock findLockForThread( Thread thread) {
		for ( AffinityLock al : LOCKS) {
			if ( al.assignedThread == thread) {
				return al;
			}
		}
		return null;
	 ...
	 *
	 * @param bind
	 *            whether to bind the thread as well
	 * @param wholeCore
	 *            whether to reserve all the thread in the same core.
	 */
	 ...
	 *
	 * @param wholeCore
	 *            if true, also reserve the whole core.
	 */
	 ...
	 * <p/>
	 * The strategies are evaluated in order to (like a search path) to find the next appropriate thread. If ANY is not
	 * the last strategy, a warning is logged and no cpu is assigned (leaving the OS to choose)
	 *
	 * @param strategies
	 *            To determine if you want the same/different core/socket.
	...
	protected void finalize() throws Throwable {
		if ( reservable) {
			LOGGER.warning( "Affinity lock for " + assignedThread
